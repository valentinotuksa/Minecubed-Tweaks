...
public class ExtraFruitDrops implements Listener {
    ...
    public void onLeafDecay(final LeavesDecayEvent event){
        ...
        switch (leaf) {
            case OAK_LEAVES, DARK_OAK_LEAVES -> getDropItem(location, Material.APPLE, 10);
            case BIRCH_LEAVES -> getDropItem(location, "minecubed:cherry", 10);
            case MANGROVE_LEAVES -> getDropItem(location, "minecubed:pitaya", 8);
            case JUNGLE_LEAVES -> getDropItem(location, "minecubed:mango", 10);
            case ACACIA_LEAVES -> getDropItem(location, "minecubed:banana", 10);
            case FLOWERING_AZALEA_LEAVES, AZALEA_LEAVES -> getDropItem(location, "minecubed:starfruit", 10);
            default -> { }
        }
    }
    
    private void getDropItem(Location location, Object item, int chance){
        if ( new Random().nextInt(100) + 1 > chance ) return;
        ...
        if (item instanceof Material) {
            ...
            itemStack = Cache.getCustomItem((String) item);
        } else {
